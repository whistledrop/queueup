...
func TestQueueNotifications(t *testing.T) {
	...
	box.waitFor(t, "Queue: about position 61", 15*time.Second) // crossed the 100 line
	box.waitFor(t, "Queue: about position 12", 15*time.Second) // crossed the 50 line
	box.waitFor(t, "Queue: about position 1", 15*time.Second)  // crossed the 10 line
	...
	allowed := map[string]bool{
		"In the queue": true, "Queue: about position 61": true, "Queue: about position 12": true,
		"Queue: about position 1": true, "You're in": true,
	}
	count := 0
